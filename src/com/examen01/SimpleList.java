...
public class SimpleList<E> {
  ...
  public SimpleList<E> intersect(SimpleList<E> anotherLst){
    SimpleList<E> intersectedList = new SimpleList<>();
    for (int i = 0; i < anotherLst.numberOfElements; i++) {
      if (this.contains(anotherLst.get(i))){
        intersectedList.addLast(anotherLst.get(i));
      }
    }
  ...
  public boolean superset(SimpleList<E> anotherLst){
    if(anotherLst.numberOfElements > this.numberOfElements)
      return false;
    int simpleListSize = anotherLst.numberOfElements;
    for (int i = 0; i < anotherLst.numberOfElements; i++) {
      if (this.contains(anotherLst.get(i))) {
        simpleListSize -= 1;
      }
    }
    return simpleListSize == 0;
  }
